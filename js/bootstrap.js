...
(function (global, factory) {
  ...
  function _defineProperties(target, props) {
    for (var i = 0; i < props.length; i++) {
     ...
     // Função para armazenar os dados do registro no localStorage
function submitRegistration() {
  ...
  if (name && number && email) {
      ...
      localStorage.setItem('userData', JSON.stringify({ name, number, email }));
...
// Função para verificar a senha e mostrar dados do usuário
function submitAccess() {
  const password = document.getElementById('access-password').value;

  if (password === 'Pop123') {
      // Exibir dados do usuário armazenados
...
function displayUserData(data) {
  ...
  userDataElement.innerHTML = `
      Nome: ${data.name}<br>
      Número: ${data.number}<br>
      E-mail: ${data.email}
  `;
}

// Função para eliminar dados e retornar ao formulário de registro
function deleteData() {
